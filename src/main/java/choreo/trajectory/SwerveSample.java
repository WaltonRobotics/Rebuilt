...
public class SwerveSample implements TrajectorySample<SwerveSample> {
  ...
  public SwerveSample interpolate(SwerveSample endValue, double timestamp) {
    ...
    //   x(τ) = xₖ + vₖτ + 1/2 aₖτ²
    //   v(τ) = vₖ + aₖτ
    double tau = timestamp - this.t;
    double tau2 = tau * tau;
    return new SwerveSample(
        timestamp,
        this.x + this.vx * tau + 0.5 * this.ax * tau2,
        this.y + this.vy * tau + 0.5 * this.ay * tau2,
        this.heading + this.omega * tau + 0.5 * this.alpha * tau2,
        this.vx + this.ax * tau,
        this.vy + this.ay * tau,
        this.omega + this.alpha * tau,
        this.ax,
        this.ay,
        this.alpha,
        interp_fx,
