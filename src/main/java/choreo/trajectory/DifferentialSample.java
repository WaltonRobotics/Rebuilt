  ...
  public DifferentialSample interpolate(DifferentialSample endValue, double timestamp) {
    ...
    BiFunction<Matrix<N6, N1>, Matrix<N3, N1>, Matrix<N6, N1>> f =
        (state, input) -> {
          ...
          //  v̇ᵣ = aᵣ
          //  ω̇ = α
          var theta = state.get(2, 0); //θ
          var vl = state.get(3, 0);
          var vr = state.get(4, 0);
          var omega = state.get(5, 0); //ω
          var al = input.get(0, 0);
          var ar = input.get(1, 0);
          var alpha = input.get(2, 0); //α
          var v = (vl + vr) / 2;
          return VecBuilder.fill(v * Math.cos(theta), v * Math.sin(theta), omega, al, ar, alpha);
        };

    double tau = timestamp - this.t; //τ
    var sample = NumericalIntegration.rkdp(f, initialState, VecBuilder.fill(al, ar, alpha), tau);
