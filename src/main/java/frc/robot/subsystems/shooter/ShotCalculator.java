...
public class ShotCalculator {
    ...
    static {
        ...
        shot.add(5.577, 57.426 + kScoringRPSBoost, 0.927, 1.4, 0.500); //1.249 // 1.18
        shot.add(4.979, 55.160 + kScoringRPSBoost, 0.866, 1.36, 0.500); //1.249 // 1.18
        shot.add(4.555, 53.548 + kScoringRPSBoost, 0.819, 1.33, 0.500); //1.153 // 1.12
        shot.add(4.231, 52.318 + kScoringRPSBoost, 0.779, 1.30, 0.500); //1.127 // 1.09
        shot.add(3.798, 51.750 + kScoringRPSBoost, 0.721, 1.29, 0.500); //1.095 // 1.08
        shot.add(3.267, 48.660 + kScoringRPSBoost, 0.641, 1.25, 0.500); //1.060 // 0.95
        shot.add(2.826, 46.986 + kScoringRPSBoost, 0.563, 1.22, 0.500); //1.035 // 0.93
        shot.add(2.212, 44.656 + kScoringRPSBoost, 0.432, 1.23, 0.500); //1.006 // 0.98
        shot.add(1.929, 43.582 + kScoringRPSBoost, 0.359, 1.14, 0.500); //0.995 // 0.87
     ...
     */
    public static Pose2d compensatePoseForLatency(Pose2d rawPose, ChassisSpeeds speeds) {
        double dt = kTotalLatencySec;

        // estimate acceleration by comparing velocity to last cycle (finite difference over 20ms)
        double ax = (speeds.vxMetersPerSecond - s_prevVx) / 0.02;
        double ay = (speeds.vyMetersPerSecond - s_prevVy) / 0.02;
        double aOmega = (speeds.omegaRadiansPerSecond - s_prevOmega) / 0.02;

        // 2nd order extrapolation: position += v*dt + 0.5*a*dt^2
        Pose2d compensated = rawPose.exp(new Twist2d(
            speeds.vxMetersPerSecond * dt + 0.5 * ax * dt * dt,
            speeds.vyMetersPerSecond * dt + 0.5 * ay * dt * dt,
            speeds.omegaRadiansPerSecond * dt + 0.5 * aOmega * dt * dt));

        // stash for next cycle so we can compute acceleration again
        s_prevVx = speeds.vxMetersPerSecond;
        s_prevVy = speeds.vyMetersPerSecond;
        s_prevOmega = speeds.omegaRadiansPerSecond;
    ...
    public static double computeShotConfidence(
        ...
        double convergenceQuality = MathUtil.clamp(solverQuality, 0, 1);
        ...
        double distanceScale = MathUtil.clamp(kHeadingReferenceDistance / Math.max(distance, 0.1), 0.5, 2.0);
    ...
    public static ShotDataLerp iterativeMovingShotFromInterpolationMap(Pose2d robot,
        ...
        tof = MathUtil.clamp(tof, kTofMin, kTofMax);
        ...
        double solverQuality = converged ? 1.0
            : MathUtil.clamp(1.0 - solverResidual / 0.1, 0, 1);
